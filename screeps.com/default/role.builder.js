...
module.exports = {
    ...
    run: function ( creep ) {
        ...
        if ( debug ) { log.output( 'Debug', 'Begin - Role Builder for ' + creep.name, true ) };
        ...
        if ( creep.memory.building ) {
            ...
            if ( aTargets.length > 0 ) {
                ...
                if ( creep.build( buildtargets[0] ) == ERR_NOT_IN_RANGE ) {
                    creep.moveTo( buildtargets[0], { visualizePathStyle: { stroke: '#2386ea' } } );
                };
        ...
        if ( debug ) { log.output( 'Debug', 'Role Builder took: ' + ( Game.cpu.getUsed() - timer ) + ' CPU Time', false, true ) };
        if ( debug ) { log.output( 'Debug', 'End - Role Builder' ) };
